...
describe('MobileTable', () => {
  ...
  describe('receives props.classes', () => {
    ...
    it('properly assigns to state.classes', () => {
      const fullMobileTable = TestUtils.renderIntoDocument(<MobileTable classes={fullProps} headers={headers} data={data}/>)
      ...
      const partialMobileTable = TestUtils.renderIntoDocument(<MobileTable classes={someProps} headers={headers} data={data}/>)
      ...
      for (let prop in expectPartialClasses) {
        if (typeof expectPartialClasses[prop] === 'object') {
          ...
          for (let prop1 in expectPartialClasses[prop]) {
            expect(partialMobileTable.state.classes[prop][prop1]).toBe(expectPartialClasses[prop][prop1])
          }
        } else {
          expect(partialMobileTable.state.classes[prop]).toBe(expectPartialClasses[prop])
        }
      }
    });
  ...
  describe('receives props.styles', () => {
    ...
    it('properly assigns to state.styles', () => {
      const fullMobileTable = TestUtils.renderIntoDocument(<MobileTable styles={fullProps} headers={headers} data={data}/>)
      for (let prop in fullProps) {
        // ewww...
        if (typeof fullProps[prop] === 'object') {
          for (let prop1 in fullProps[prop]) {
      ...
      const partialMobileTable = TestUtils.renderIntoDocument(<MobileTable styles={someProps} headers={headers} data={data}/>)
      ...
      for (let prop in expectPartialStyles) {
        if (typeof expectPartialStyles[prop] === 'object') {
          ...
          for (let prop1 in expectPartialStyles[prop]) {
            expect(partialMobileTable.state.styles[prop][prop1]).toBe(expectPartialStyles[prop][prop1])
          }
        } else {
          expect(partialMobileTable.state.styles[prop]).toBe(expectPartialStyles[prop])
        }
      }
    });
    ...
    it('properly assigns to inline styles', () => {
      ...
      let table = TestUtils.scryRenderedDOMComponentsWithTag(fullMobileTable, 'table')
      ...
      let thead = TestUtils.scryRenderedDOMComponentsWithTag(fullMobileTable, 'thead')
      for (let prop in fullProps.table) {  
      ...
      let tbody = TestUtils.scryRenderedDOMComponentsWithTag(fullMobileTable, 'tbody')
      ...
      */
      ...
      table = TestUtils.scryRenderedDOMComponentsWithTag(partialMobileTable, 'table')
      ...
      thead = TestUtils.scryRenderedDOMComponentsWithTag(partialMobileTable, 'thead')
      for (let i = 0; i < thead.length; i++) {
      ...
      tbody = TestUtils.scryRenderedDOMComponentsWithTag(partialMobileTable, 'tbody')
      for (let i = 0; i < tbody.length; i++) {
